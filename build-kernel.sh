...
function extract-extra-version-number() {
	...
	local vnum
	vnum="$(git describe)"
	if [[ "$vnum" =~ $version_or_rc ]]; then
		echo ""
	elif [[ "$vnum" =~ $inbetween_version ]]; then
		echo "$vnum" | cut -f2 -d-
	elif [[ "$vnum" =~ $inbetween_rc ]]; then
		echo "$vnum" | cut -f3 -d-
	fi
}
...
function configure-kernel() {
    ...
    PREIMAGE_CONFIG="$(fdfind 'config-\d\.\d+\.\d+-(amd64|\d+-generic)' /boot --max-depth=1 --type f | sort | tail -n1)"
